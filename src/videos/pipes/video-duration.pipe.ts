...
export class VideoDurationValidationPipe implements PipeTransform {
    ...
    async transform(value: Express.Multer.File, metadata: ArgumentMetadata) {
        ...
        const maxDuration = this.configService.get<number>('MAX_DURATION_IN_SECONDS', 30);
        ...
        return new Promise((resolve, reject) => {
            ffmpeg.ffprobe(value.path, (err, metadata) => {
                ...
                if (durationInSeconds > maxDuration) {
                    reject(new BadRequestException(`Video duration exceeds the maximum allowed duration of ${maxDuration} seconds`));
                } else {
                    resolve(value);
                }
            });
